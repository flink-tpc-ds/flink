...
class BatchTestBase extends BatchAbstractTestBase {
  ...
  def before(): Unit = {
    ...
    conf.getConfiguration.setInteger(ExecutionConfigOptions.SQL_RESOURCE_HASH_AGG_TABLE_MEM, 50)
    conf.getConfiguration.setInteger(ExecutionConfigOptions.SQL_RESOURCE_HASH_JOIN_TABLE_MEM, 50)
